...
| App operational logs; `app/routes/internal.py` | Error/operational text to host, **plus source-IP on auth-failure only** | Operations; Art. 6(1)(f) where personal, Art. 6(1)(f) legitimate interest (attack/brute-force detection) for the IP-on-failure case specifically | Doc-drift audit correction (verified against real code, not this row's prior text): a later, legitimate security fix (`security_review_owasp.md`) re-added `source_ip` logging on both the resync-token and webhook signature-failure paths — a deliberate, considered trade-off (detecting attack patterns) reintroducing exactly what this row previously said was removed. Logged only on auth *failure*, never on a successful request; container rotation is a size limit, not a timed deletion guarantee; set actual retention |
