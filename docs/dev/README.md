...
| `bulliexplorer_experience_2027.md` | "Explore the journey, not just the post" repositioning — map/route/POI-first navigation instead of a post-list-first blog. Phased (P0-P4), each phase with its own **Done when** and honest partial/not-started status — consult before picking up the next phase rather than re-deriving scope from the original external review. |
...
| `fix_peaks_cablecars_amenity_review.md` | Peak elevation labels (real tile inspection corrected the doc's own `physical_point` assumption — peaks actually live on `pois` as `kind="peak"`; native basemap already renders the icon+name, this fix adds only the elevation text). Cable cars: real tile inspection at the actual Feldbergbahn found zero aerialway data — closed, not built. New amenity categories `cafe`/`bicycle_repair_station` added to Overpass auto-discovery, verified against the real live Overpass API. **Regression #2, found live**: the elevation-text layer's own presence was silently suppressing the native peak label entirely via an undocumented MapLibre collision interaction — fixed with `text-allow-overlap`/`text-ignore-placement`, verified with a real headless browser using the literal fixed function against production. **Regression #3, found live at a denser location (Siebengebirge)**: fix #2 traded one problem for another — orphaned bare elevation numbers for minor, unnamed peaks the native layer doesn't consider prominent enough to label — fixed by adding the native layer's own per-feature `min_zoom` prominence gate to this layer's filter, verified with a real headless browser at both a sparse and a dense real location. |
...
| `playwright_e2e_smoke_tests.md` | A third test tier (`tests/e2e/`) — real Postgres, a real `uvicorn` process, a real Chrome — direct response to the filter-dialect regression: every existing `post-map.js` test only checks strings exist, none execute JS. Verified against the exact broken commit: the new test fails with the real MapLibre error strings. Found and fixed two real, pre-existing bugs along the way: the actual GitHub Actions CI had been failing on every push this session (no migrations applied to the ephemeral service container — `make ci` run locally never caught it since it always used the already-migrated persistent dev DB), and this tier's own fixture seeding initially deleted the real local dev posts (`sync_posts()`'s reconciling delete isn't scoped to its given directory) — fixed with a dedicated `<db>_e2e` isolated database. |
| `cyclosm_optional_layer.md` | First genuinely external third-party raster tile dependency — an off-by-default CyclOSM overlay toggle. Researched (not assumed) that MapLibre GL JS has no built-in tile timeout, then verified empirically with a real headless browser that a caller-supplied `AbortSignal` via `transformRequest` genuinely aborts the fetch in this exact vendored build — the Phase 2 timeout is built on that confirmed mechanism. Fixed a genuine z-order bug in the theme-swap carry-over (CyclOSM needs the opposite append order from route/amenities). Verified against the doc's own real reference example (Hohenzollernbrücke, Köln — real street names, EuroVelo badges, a ferry line) and a real total-failure simulation. Also found and fixed an unrelated, real, blocking local-dev-DB migration gap discovered along the way. |
| `fix_lcp_image_and_static_cache.md` | Two real PageSpeed findings: LCP 15.1s, and the single largest saving (2.5MB) from cache lifetimes, not image weight. Cover images now processed once at sync time (Pillow resize+WebP, real 89-92% size reductions on production images, verified via re-fetching the actual uploaded R2 object). Static assets version-stamped (`?v=` content hash) with aggressive immutable caching, `no-cache` kept as the fallback — both verified live with `curl` against production. Real before/after Lighthouse comparison (3 runs each side, against the live production URL, since Google's own PSI API was rate-limited and its web UI is JS-rendered): LCP roughly halved, TBT down ~99% — but FCP got measurably *worse* on route/map pages specifically, reported honestly as an unresolved, characterized-not-fixed anomaly rather than glossed over. |
